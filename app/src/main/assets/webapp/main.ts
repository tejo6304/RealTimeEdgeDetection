...
class FilteredImageGallery {
    ...
    private async loadImages(): Promise<void> {
        try {
            ...
            if ((window as any).ImageGalleryBridge) {
                const jsonData = (window as any).ImageGalleryBridge.getImages();
    ...
    private createImageCard(image: CapturedImage): HTMLElement {
    ...
    private goBackToCamera(): void {
        // Check if Android bridge is available
        if ((window as any).ImageGalleryBridge) {
            (window as any).ImageGalleryBridge.goBack();
        } else {
