...
class FrameViewer {
    constructor() {
        ...
        this.fps = 0;
        this.canvasElement = this.getElement('frame-canvas');
        this.ctx = this.getContext(this.canvasElement);
        this.statsElement = this.getElement('stats-overlay');
        this.overlayCanvas = this.getElement('overlay-canvas');
        this.overlayCtx = this.getContext(this.overlayCanvas);
        this.videoStream = this.getElement('video-stream');
        this.outputCanvas = this.getElement('output-canvas');
    ...
    processFramesContinuous() {
        ...
        try {
            ...
            this.ctx.drawImage(this.videoStream, 0, 0, this.canvasElement.width, this.canvasElement.height);
            // Store image data
    ...
    refreshStatsPanel() {
        ...
        if (statRows.length > 0) {
            const filterRow = Array.from(statRows).find((row) => row.textContent?.includes('Filter:'));
            if (filterRow) {
     ...
     */
    applyGrayscaleFilterCanvas() {
        if (!this.originalImageData)
            return;
        const imageData = new ImageData(new Uint8ClampedArray(this.originalImageData.data), this.originalImageData.width, this.originalImageData.height);
    ...
    applyCannyEdgeFilterCanvas() {
        ...
        for (let y = 1; y < height - 1; y++) {
            for (let x = 1; x < width - 1; x++) {
                ...
                // Sobel X
                const gx = -1 * this.getPixel(data, x - 1, y - 1, width) +
                    ...
                    1 * this.getPixel(data, x + 1, y + 1, width);
                // Sobel Y
                const gy = -1 * this.getPixel(data, x - 1, y - 1, width) +
